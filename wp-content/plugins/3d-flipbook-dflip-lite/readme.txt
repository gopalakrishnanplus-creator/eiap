...
Plugin Name: DearFlip - PDF Flipbook, 3D Flipbook, PDF embed, PDF viewer
Author: dearhive
...
Introducing DearFlip, the user-friendly 3D flipbook WordPress plugin. Provide the PDF link, and DearFlip will transform your PDF into an immersive 3D flipbook. It's the simplest solution for creating captivating flipbooks in WordPress. Choose from various flipbook styles, including 3D flipbook, 2D flipbook, image flipbook, PDF flipbook, and portfolio flipbooks.

**Engage with Dynamic 3D Flipbooks!** Transform your flat PDFs into captivating and immersive 3D Flipbooks. With DearFlip, you can effortlessly create a visually stunning and realistic 3D Flipbook presentation that will grab your audience's attention. Say goodbye to static documents and bring your PDFs to life!
...
### Viewer Modes available in DearFlip
- **3D Flipbook**- With DearFlip, your PDFs and images will be transformed into interactive, lifelike books. This is achieved through cutting-edge WebGL technology, which does all the complex work behind the scenes. Meanwhile, your customers will be enjoying your PDF content with delight. Our 3D Flipbook is a source of pride for us, and we believe it will soon be for you as well.
...
- **PDF embed and PDF Viewer** - For those who prefer classic PDF viewers, DearFlip offers a feature that allows you to create PDF embeds on your page that mimic the look and feel of a browser's embedded PDF viewer. This option is particularly suitable for documents with irregular layouts or formatting.
...
### File Formats Supported by DearFlip
...
### How easy is DearFlip? Just 5 Steps! ###
Known for its user-friendly interface, WordPress serves as an inspiration for DearFlip's post-structure when creating and managing 3D flipbooks. The process is designed to be as straightforward and effortless as creating a regular post, making it accessible to users of all skill levels.
...
* **Slider Mode** displays PDF pages as slides, just like in Slideshare.
...
* **Deep linking** to specific pages of a flipbook via the share button, enabling easy sharing and referencing.
...
### PRO-FEATURES ###
* PDF links – links inside your PDFs are clickable (Test your PDF links without a premium version using our online [PDF Flipbook Viewer](https://dearflip.com/go/wp-lite-pdf-viewer) to get the idea)
...
* Controls Customization - chose what controls to hide/display in your flipbook
...
### DearFlip is also available beyond WordPress! ###
DearFlip, accessible across multiple platforms, proudly serves a diverse user base exceeding 200,000 individuals. These users span various niches, each with unique interests and needs.
- 200,000+ active installations on WordPress plugin repository [4.8/5 based on 112 ratings](https://wordpress.org/support/plugin/3d-flipbook-dflip-lite/reviews/)
...
> Note: When upgrading to the PRO version of DearFlip 3D flipbook WordPress plugin, make sure you deactivate the lite version. Your flip book created with the lite version will stay in the database and work seamlessly. If you are using a cache plugin, we recommend clearing the cache if any issues arise.
...
= Loading Case 2: Loading Icon and Loading Message: =
...
= How to install DearFlip on my WordPress site? =
...
DearFlip - 3D Flipbook uses PDF.js to render the PDF pages. At any given time it requires only six pages of data at max. So the number of pages in a PDF file doesn't really affect the 3D Flipbook. Also, it only loads six Pages of data to start the flipbook and this makes it a resource-friendly and traffic-friendly 3D flipbook. Other 3D flipbooks might crash with huge files, DearFlip will last longer.
> [Trom Site](https://www.tromsite.com/) is one of our premium clients, and one of their books has **1144 Pages and is 532MB** in size. It loads with only 4-5MB of PDF data.
...
2. Enter the title of the PDF flipbook in the Title box
...
1. Goto **'dFlip Books > Add New Book'**
2. Enter the title of the 3D Flipbook in the Title box
...
1. PDF links are a premium feature in DearFlip and won't be displayed in the lite version of DearFlip - 3D Flipbook WordPress Plugin. You can test your PDF links to check how they work in our online [PDF Flipbook Viewer](https://dearflip.com/go/wp-lite-pdf-viewer).
2. Also pay attention that some desktop readers' auto convert URL texts to links and give an assumption that those are links. Such links won't work in every reader and device. One of the popular cases is Adobe PDF Reader: it auto-converts the text URLs to links in desktop but not in mobile apps. So we suggest defining links explicitly and not relying on the PDF reader's auto conversion capability.
...
We recommend using lightbox for the PDF viewer instead of the PDF embed viewer. Using embed can result in extra bandwidth for the PDF documents even if the user may not view the document.
...
Please report security bugs found in the source code of the DearFlip plugin through the [Patchstack Vulnerability Disclosure Program](https://patchstack.com/database/vdp/9e5fb649-7332-4ed1-a07f-439b41fd71b2). The Patchstack team will assist you with verification, CVE assignment, and notify the developers of this plugin.

== Screenshots ==

1. 3D Flipbook animation of DearFlip. It's not a CGI, it's a real live screenshot.
2. Photography Flipbook with a custom image background. You can set a custom background to flipbook post, and the 3D flipbook will merge into the background.
3. Realistic 3D Flipbook created with a PDF file. PDF file used is [MeatAtlas](https://www.boell.de/en/meat-atlas)
...
* Fix: PageMode issue in documents with 1 or 2 pages
* Add: WordPress 5.9 tested
...
= 2.3.74: =
* Improve: Code updated as per Plugin Check and WordPress Coding Standards

= 2.3.67: =
* Improve: ShareBox is added to DOM only when required
...
= 2.3.48: =
* Add: Compatibility checked with WordPress 6.7
