...
class BaseStripeClient implements StripeClientInterface, StripeStreamingClientInterface
{
    ...
    /** @var \StellarWP\Learndash\Stripe\Util\RequestOptions */
    private $defaultOpts;
    ...
    public function __construct($config = [])
    {
        if (\is_string($config)) {
            ...
            throw new \StellarWP\Learndash\Stripe\Exception\InvalidArgumentException('$config must be a string or an array');
        }
        ...
        $this->defaultOpts = \StellarWP\Learndash\Stripe\Util\RequestOptions::parse([
    ...
    public function request($method, $path, $params, $opts)
    {
        ...
        $requestor = new \StellarWP\Learndash\Stripe\ApiRequestor($this->apiKeyForRequest($opts), $baseUrl);
    ...
    public function requestCollection($method, $path, $params, $opts)
    {
        ...
        if (!($obj instanceof \StellarWP\Learndash\Stripe\Collection)) {
            ...
            throw new \StellarWP\Learndash\Stripe\Exception\UnexpectedValueException($msg);
        }
    ...
    {
        $obj = $this->request($method, $path, $params, $opts);
        if (!($obj instanceof \StellarWP\Learndash\Stripe\SearchResult)) {
            $received_class = \get_class($obj);
    ...
    /**
     * @param \StellarWP\Learndash\Stripe\Util\RequestOptions $opts
     *
     * @throws \StellarWP\Learndash\Stripe\Exception\AuthenticationException
     *
     * @return string
     */
    private function apiKeyForRequest($opts)
    {
        ...
        if (null === $apiKey) {
            ...
            throw new \StellarWP\Learndash\Stripe\Exception\AuthenticationException($msg);
        }
     ...
     *
     * @throws \StellarWP\Learndash\Stripe\Exception\InvalidArgumentException
     */
    private function validateConfig($config)
    {
        // api_key
        if (null !== $config['api_key'] && !\is_string($config['api_key'])) {
            throw new \StellarWP\Learndash\Stripe\Exception\InvalidArgumentException('api_key must be null or a string');
        }

        if (null !== $config['api_key'] && ('' === $config['api_key'])) {
            $msg = 'api_key cannot be the empty string';

            throw new \StellarWP\Learndash\Stripe\Exception\InvalidArgumentException($msg);
        }
        ...
        if (null !== $config['client_id'] && !\is_string($config['client_id'])) {
            throw new \StellarWP\Learndash\Stripe\Exception\InvalidArgumentException('client_id must be null or a string');
        }

        // stripe_account
        if (null !== $config['stripe_account'] && !\is_string($config['stripe_account'])) {
            throw new \StellarWP\Learndash\Stripe\Exception\InvalidArgumentException('stripe_account must be null or a string');
        }

        // stripe_version
        if (null !== $config['stripe_version'] && !\is_string($config['stripe_version'])) {
            throw new \StellarWP\Learndash\Stripe\Exception\InvalidArgumentException('stripe_version must be null or a string');
        }

        // api_base
        if (!\is_string($config['api_base'])) {
            throw new \StellarWP\Learndash\Stripe\Exception\InvalidArgumentException('api_base must be a string');
        }

        // connect_base
        if (!\is_string($config['connect_base'])) {
            throw new \StellarWP\Learndash\Stripe\Exception\InvalidArgumentException('connect_base must be a string');
        }

        // files_base
        if (!\is_string($config['files_base'])) {
            throw new \StellarWP\Learndash\Stripe\Exception\InvalidArgumentException('files_base must be a string');
        }
        ...
        if (!empty($extraConfigKeys)) {
            ...
            throw new \StellarWP\Learndash\Stripe\Exception\InvalidArgumentException('Found unknown key(s) in configuration array: ' . $invalidKeys);
        }
    }
}
