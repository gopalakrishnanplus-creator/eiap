...
use StellarWP\Learndash\Stripe\StripeObject;
...
abstract class Util
{
    ...
    {
        $types = \StellarWP\Learndash\Stripe\Util\ObjectTypes::mapping;
        if (self::isList($resp)) {
            $mapped = [];
            foreach ($resp as $i) {
        ...
        if (\is_array($resp)) {
            if (isset($resp['object']) && \is_string($resp['object']) && isset($types[$resp['object']])) {
                ...
                $class = \StellarWP\Learndash\Stripe\StripeObject::class;
            }
    ...
    {
        if ($h instanceof \StellarWP\Learndash\Stripe\ApiResource) {
            return $h->id;
        }
        if (static::isList($h)) {
            $results = [];
