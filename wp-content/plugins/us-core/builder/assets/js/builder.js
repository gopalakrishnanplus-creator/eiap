...
! function( $, undefined ) {
	...
	*/
	...
	var USBuilder = function( container ) {
		...
		self.$panelAddItemsTabs = $( '.us-builder-panel-add-items', self.$panel );
		// Panel Actions
		...
		[
			...
			// Event handlers for templates tab
			'loadTemplatesConfig',
		...
		[
			...
			'redoChange',		// the recovery data handler from preview page (command|ctrl+v)
			'undoChange'		// the recovery data handler from preview page (command|ctrl+z)
		].map( function( method ) {
		...
		self.$panel
			...
			.on( 'input', 'input[name=search]', $ush.debounce( self._events.searchPanelElms, 1 ) )
	...
	$.extend( $usbPrototype, {
		...
		initElmFieldset: function( id, callback ) {
			...
			var name = self.getElmName( id ),
				elmTitles = self.config( 'elm_titles', /* default */{} );

			// If there is no title, then the element does not support edit in the us-builder
			if (
				! $.isPlainObject( elmTitles )
				|| ! elmTitles.hasOwnProperty( name )
			) {
			...
			if ( ! self._elmsFieldset[ name ] && ! self._$temp.isFieldsetsLoaded ) {
				self.setPanelTitle( elmTitles[ name ] );
		...
		_switchPanel: function() {
			...
			self.$panel.toggleClass( 'hide', isHide );

			if ( ! isHide && self.selectedElmId ) {
		...
		_showPanelPageSettings: function () {
			...
			if ( ! ( self._fieldsets.pageMeta instanceof $usof.GroupParams ) ) {
				var pageMeta = new $usof.GroupParams( $( '.us-builder-panel-page-meta', self.$panelPageSettings )[/* first */0] );
	...
	$.extend( $usbPrototype, {
		...
		/**
		 * Determines whether the specified id is outside container
		 *
		 ...
		 */
		isOutsideContainer: function( id ) {
			var self = this;
			return $usbcore.indexOf( self.getElmName( id ), self.config( 'outside_elms', [] ) ) > -1;
		},
