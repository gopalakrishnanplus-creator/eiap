...
if ( ! function_exists( 'us_api' ) ) {
	 ...
	 * Note: The function is duplicated in `common/admin/functions/functions.php`
	 *
	...
	function us_api( $url, $get_variables = array(), $as_array = FALSE, $us_api = TRUE ) {
		...
		$us_themename = defined( 'US_ACTIVATION_THEMENAME' )
			...
			: US_THEMENAME;
		$url = str_replace( ':us_themename', us_strtolower( $us_themename ), $url );
...
if ( ! function_exists( 'us_get_current_id' ) ) {
	...
	function us_get_current_id() {
		...
		return (int) apply_filters( 'us_tr_object_id', $current_id );
	}
}
...
if ( ! function_exists( 'us_get_custom_field' ) ) {
	 ...
	 */
	function us_get_custom_field( $name, $acf_format = TRUE ) {
...
if ( ! function_exists( 'us_get_post_content' ) ) {
	 ...
	 * @param bool $use_cache Use cached value or get a content directly from post data
	 ...
	 */
	function us_get_post_content( $post, $get_nested_blocks = TRUE, $use_cache = TRUE, $current_level = 1 ) {
		...
		if ( $use_cache AND ( $post_content = wp_cache_get( $post_ID, __FUNCTION__ ) ) !== FALSE ) {
			return $post_content;
		}
		...
		if ( $get_nested_blocks AND ! empty( $post_content ) ) {
			 ...
			 */
			$func_get_shortcode_content = function( $matches ) use ( $use_cache, $current_level ) {
				...
				if ( $current_id > 0 ) {
					return us_get_post_content( $current_id, /* get_nested_blocks */TRUE, $use_cache, ++$current_level );
				}
		...
		if ( $use_cache ) {
			wp_cache_set( $post_ID, $post_content, __FUNCTION__ );
		}
...
if ( ! function_exists( 'us_generate_link_atts' ) ) {
	...
	function us_generate_link_atts( $link = '' ) {
		...
		// Get a link from a custom field
		if (
			isset( $result['url'] )
			AND preg_match( "/{{([^}]+)}}/", trim( rawurldecode( $result['url'] ) ) )
		) {
			// Get a custom field value without ACF return Format
			$meta_value = us_get_custom_field( $result['url'], FALSE );
			...
			if ( is_scalar( $meta_value ) ) {
				...
				if ( strpos( $meta_value, '{' ) === 0 ) {
					try {
						$meta_value_array = json_decode( $meta_value, /* to array */TRUE );
					...
					// If the value is numeric, use it as post or attachment ID
				} elseif ( is_numeric( $meta_value ) ) {
					$result['url'] = ( get_post_type( (int) $meta_value ) == 'attachment' )
				...
				// When the value is an array use it itself as a result (ACF types: "Link", "File")
			} else {
				$result = $meta_value;
			}
		}
...
if ( ! function_exists( 'us_get_demo_import_config' ) ) {
	...
	function us_get_demo_import_config() {
		...
		$us_api_response = us_api( '/demos_config/:us_themename', $get_variables, /* return as_array */TRUE );
...
if ( ! function_exists( 'us_replace_dynamic_value' ) ) {
	 ...
	 * @return string
	 */
	function us_replace_dynamic_value( $string ) {
		$pattern = '/{{([^}]+)}}/';
		...
		return (string) preg_replace_callback( $pattern, function ( $matches ) {
			...
			if ( ( $current_id = us_get_current_id() ) < 1 ) {
				return ''; // Error, unable to determine current id
			}
			...
			if ( $matches[/* code */0] == '{{comment_count}}' AND us_get_current_meta_type() == 'post' ) {
				...
				// Predefined: change '{{the_title}}' to total users amount
			} elseif ( $matches[/* code */0] == '{{the_title}}' ) {
				return strip_tags( do_shortcode( '[us_post_title]' ) );

				// Get the custom field value
			} else {
				$meta_value = us_get_custom_field( $matches[/* name */1] );
	...
	function us_conditions_are_met( $conditions, $conditions_operator ) {
		...
		foreach( $conditions as $i => $condition ) {
			...
			if ( $condition_param === 'post_type' ) {
				...
				$condition_result = $func_compare_values(
					$meta_value,
					us_arr_path( $condition, 'cf_value', '' ),
