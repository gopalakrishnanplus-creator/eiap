...
return null}}(jQuery);jQuery.fn.usMod=function(mod,value){if(this.length==0){return this}
...
else{var regexp=new RegExp('(^| )'+mod+'\_[a-zA-Z0-9\_\-]+( |$)');return this.each(function(){if(this.className.match(regexp)){this.className=this.className.replace(regexp,'$1'+mod+'_'+value+'$2')}else{this.className+=' '+mod+'_'+value}}).trigger('usof.'+mod,value)}};window.$ush=window.$ush||{};!function($,undefined){var _window=window,_document=document,_undefined=undefined;if(_window.$usof===_undefined){_window.$usof={}}
if($usof.mixins===_undefined){$usof.mixins={}}
$usof.mixins.Events={on:function(eventType,handler){var self=this;if(self.$$events===_undefined){self.$$events={}}
...
return self._dynamicColors};$usof.field=function(row,options){var self=this;self.$document=$(_document);self.$row=$(row);self.$responsive=$('> .usof-form-row-responsive',self.$row);var data=self.$row.data()||{};self.type=self.$row.usMod('type');self.id=data.id;self.uniqid=$ush.uniqid();self.name=data.name;self.inited=data.inited||!1;self.$input=$('[name="'+data.name+'"]:not(.js_hidden)',self.$row);if(self.inited){return}
...
self._events.change=function(){self.trigger('change',[self.getValue()])};self.$input.on('change',self._events.change);return self},isEditLive:function(){var self=this;return!!(self.$row.closest('.us-builder-panel-fieldset').length||self.$row.parents('.us-builder-panel-body').length)},initResponsive:function(){var self=this;if(!self.hasResponsive()){return}
...
return $ush.rawurlencode(JSON.stringify(value))},toPlainObject:function(value){try{value=JSON.parse($ush.rawurldecode(''+value)||'{}')}catch(e){value={}}
...
var value=this.getValue();$usof.instance.valuesChanged[this.groupName]=value;this.$field.trigger('change',value)}},getDefaultValue:function(){var self=this;return(self._std!==_undefined)?self._std:''},setValue:function(value){if(typeof value==='string'&&this.hasStringValue){try{value=JSON.parse($ush.rawurldecode(value)||'[]')}catch(err){console.error(value,err);value=[]}}
...
groupParams.fields[fieldId].trigger('change');break}}.bind(this));this._reInitParams()},getValue:function(){var result=[];$.each(this.groupParams,function(i,groupParams){result.push(groupParams.getValues())});if(this.hasStringValue){try{result=$ush.rawurlencode(JSON.stringify(result))}catch(err){console.error(result,err);result=''}}
...
if(light<178){this.$input.addClass('white')}else{this.$input.removeClass('white')}}}}(jQuery);!function($,undefined){var _window=window;_window.$ush=_window.$ush||{};if($ush.isUndefined(_window.$usof)){return}
...
var insertItem=function($el,name,value){if(!this.items.hasOwnProperty(value)){var text=(name||'').replace(/\s/,'').toLowerCase(),$item=$('<div data-value="'+$ush.stripTags(value)+'" data-text="'+$ush.stripTags(text)+'" tabindex="3">'+name+'</div>');$el.append($item);this.items[value]=$item}};$.get(ajaxurl,data,function(res){this.loaded=!1;this.$container.removeClass('loaded');this._clearMessage.call(this);if(!res.success){this._showMessage.call(this,res.data.message);return}
...
$usof.field.text=$usof.field.textarea={init:function(){var self=this;self.$row.on('click','.usof-example',self._setExampleValue.bind(self));self.$input.on('change paste keyup',$ush.debounce(function(){self.trigger('change',[self.getValue()])}))},_setExampleValue:function(e){this.setValue($(e.target).closest('.usof-example').html()||'')}}}(jQuery);!function($,undefined){var _window=window,_undefined=undefined;if(_window.$usof===_undefined){return}
...
this.__checkChangeValues=$ush.debounce(this.checkChangeValues.bind(this),1);if(this.extStates.length){$('[data-responsive-state-content="default"]',this.$container).each(function(_,content){var $content=$(content);this.extStates.map(function(responsiveState){var $cloneContent=$content.clone().attr('data-responsive-state-content',responsiveState).addClass('hidden');$content.after($cloneContent)}.bind(this))}.bind(this))}
...
field.trigger('beforeShow').on('change',$ush.debounce(this._events.changeValue.bind(this),1))}.bind(this))}.bind(this));var pid=setTimeout(function(){if(!this.inited){this.setValue(this.$input.val());this.checkChangeValues.call(this)}
...
resultValue[responsiveState][param]=value}}.bind(this))}.bind(this));resultValue=(JSON.stringify(resultValue)!=='{}')?$ush.rawurlencode(JSON.stringify(resultValue)):'';this.$input.val(resultValue);if(!this._lastResultValue||this._lastResultValue!==resultValue){this._lastResultValue=resultValue;this.trigger('change',resultValue)}
