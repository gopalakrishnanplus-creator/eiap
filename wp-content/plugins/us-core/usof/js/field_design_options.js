...
! function( $, undefined ) {
	...
	$usof.field[ 'design_options' ] = {
		init: function() {
			 ...
			 */
			this.__checkChangeValues = $ush.debounce( this.checkChangeValues.bind( this ), 1/* 1ms */ );
			...
			$.each( this.groupParams, function( responsiveState, groupParams ) {
				...
				$.each( groupParams.fields, function( _, field ) {
					...
					field
						.trigger( 'beforeShow' )
						.on( 'change', $ush.debounce( this._events.changeValue.bind( this ), 1 ) );
				}.bind( this ) );
			}.bind( this ) );
		...
		_events: {
			...
			changeValue: function( field ) {
				...
				resultValue = ( JSON.stringify( resultValue ) !== '{}' )
					...
					? $ush.rawurlencode( JSON.stringify( resultValue ) )
					: '';
