...
if (
	us_get_option( 'block_third_party_files', 0 )
	...
	if ( $provider == 'google' ) {
		...
		$learn_more_link = 'https://policies.google.com/privacy';

	} else {
		$provider_name = 'OpenStreetMap Foundation';
		$learn_more_link = 'https://wiki.osmfoundation.org/wiki/Privacy_Policy';
	}

	$output .= '<div class="w-map-caption">';
	$output .= '<p>';
	$output .= sprintf( strip_tags( __( 'By loading this map, you agree to the privacy policy of %s.', 'us' ) ), '<a href="' . esc_attr( $learn_more_link ) . '" target="_blank" rel="nofollow noopener noreferrer">' . $provider_name . '</a>' );
	$output .= '</p>';

	// Button attributes
	$btn_atts = array(
		'class' => 'w-btn us-btn-style_' . us_maybe_get_button_style() . ' action_confirm_load',
		'type' => 'button',
	);

	$output .= '<p><button ' . us_implode_atts( $btn_atts ) . '>';
	...
	$output .= '<small>' . strip_tags( __( 'Always load maps on this site', 'us' ) ) . '</small>';
	$output .= '</label></p>';

	$output .= '</div>'; // .w-map-caption
