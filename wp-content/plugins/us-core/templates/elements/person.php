...
foreach ( $social_links as $type => $title ) {
	...
	// For the builder, display empty links hidden
	if ( empty( $social_link ) AND ! usb_is_preview_page() ) {
		continue;
	}
	...
	// For the builder, display empty links hidden
	if ( empty( $social_link ) AND usb_is_preview_page() ) {
		$social_links_atts['class'] .= ' hidden';
	}
...
// Custom social link
$_has_custom_link = ( ! empty( $custom_icon ) AND ! empty( $custom_link ) );
if ( $_has_custom_link OR usb_is_preview_page() ) {
	...
	// For the builder, display empty links hidden
	if ( usb_is_preview_page() AND ! $_has_custom_link ) {
		$custom_link_atts['class'] .= ' hidden';
	}
