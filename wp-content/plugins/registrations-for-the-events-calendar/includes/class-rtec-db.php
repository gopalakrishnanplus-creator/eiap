...
class RTEC_Db {
	...
	public function insert_entry( $data, $field_attributes, $from_form = true ) {
		...
		$status     = isset( $data['status'] ) ? $data['status'] : 'c';
	...
	public function update_statuses( $ids = null ) {
		...
		if ( null !== $ids ) {
			...
			// Nothing to update for an empty event id list.
			if ( empty( $ids ) ) {
			...
			$query     .= ' AND event_id IN ( ' . $ids_string . ' )';
			$wpdb->query( $query );
		} else {
	...
	public function get_registrants_data( $event_meta, $attendee_list_fields = array() ) {
		...
		if ( isset( $rtec_options['registrants_data_who'] ) && $rtec_options['registrants_data_who'] === 'users_and_confirmed' ) {
			// "Reviewed" = only show registrations that existed at or before the most recent admin view.
			$last_viewed = function_exists( 'rtec_get_max_admin_last_viewed_registrations_at' )
				? rtec_get_max_admin_last_viewed_registrations_at()
				: 0;
			if ( ! empty( $last_viewed ) && is_numeric( $last_viewed ) ) {
				$args['where'][] = array(
					'registration_date',
					gmdate( 'Y-m-d H:i:s', (int) $last_viewed ),
					'<=',
					'string',
				);
			} else {
				// No admin has viewed registrations yet: show none for "Reviewed".
				$args['where'][] = array(
					'registration_date',
					'1970-01-01 00:00:00',
					'<=',
