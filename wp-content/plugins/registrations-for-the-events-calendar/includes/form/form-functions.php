...
function rtec_output_message_after_submit( $status, $submission ) {
	...
	if ( $status === 'form' ) {
		...
		if ( is_user_logged_in() && current_user_can( 'manage_options' ) && function_exists( 'rtec_get_attendees_dashboard_url' ) ) {
			$data     = $submission->get_data();
			$event_id = isset( $data['event_id'] ) ? absint( $data['event_id'] ) : 0;
			if ( $event_id ) {
				$dashboard_url = rtec_get_attendees_dashboard_url( $event_id );
				if ( $dashboard_url ) {
					$message_html .= '<p class="rtec-success-message rtec-admin-dashboard-link tribe-events-notices">';
					$message_html .= esc_html__( 'This message is only visible to administrators. You can view and manage this registration in your dashboard.', 'registrations-for-the-events-calendar' );
					$message_html .= ' <a href="' . esc_url( $dashboard_url ) . '" target="_blank" rel="noopener noreferrer" class="rtec-button rtec-button-dashboard">' . esc_html__( 'View in dashboard', 'registrations-for-the-events-calendar' ) . '</a>';
					$message_html .= '</p>';
				}
			}
		}
	...
	if ( ! is_admin() && isset( $_GET['action'] ) && $_GET['action'] === 'unregister' ) {
		...
		if ( $event_id !== 0 && $form->registration_deadline_has_passed() ) {
			if ( method_exists( 'Tribe__Notices', 'set_notice' ) ) {
				Tribe__Notices::set_notice( 'unregistered', __( 'You can no longer cancel your registration. The deadline has passed.', 'registrations-for-the-events-calendar' ) );
			}
		}
	}
}
...
function rtec_new_user_notice() {
	...
	<div class="rtec-new-user-notice rtec-new-user-notice-hide">
		<div class="rtec-new-user-notice-icon">
			<img src="<?php echo esc_url( rtec_plugin_url( 'assets/images/RU-Logo-150.png' ) ); ?>" width="45" height="45" alt="<?php esc_attr_e( 'Roundup WP', 'registrations-for-the-events-calendar' ); ?>" />
		</div>
		<div class="rtec-new-user-notice-text">
			<h3><?php esc_html_e( "You're viewing this event while logged in.", 'registrations-for-the-events-calendar' ); ?></h3>
			<p>
				<?php
				esc_html_e( "Your name and email are automatically filled in, and the system will detect if you've already registered for this event.", 'registrations-for-the-events-calendar' );
				?>
			</p>
		</div>
...
<script type="text/javascript">
	jQuery(document).ready(function($) {
		<?php echo wp_unslash( $rtec_custom_js ) . "\r\n"; ?>
	});
</script>
		<?php
	}
...
function rtec_custom_css() {
	...
	if ( ! empty( $rtec_custom_css ) ) {
		...
		if ( ! empty( $rtec_custom_css ) ) {
			echo wp_unslash( $rtec_custom_css ) . "\r\n";
		}
		echo '</style>' . "\r\n";
	}
}
...
function rtec_scripts_and_styles() {
	...
	if ( $enqueue_legacy_styles ) {
		wp_enqueue_style( 'rtec_styles_legacy', rtec_frontend_asset_url( 'assets/frontend/css/rtec-styles-legacy.css' ), array( 'rtec_styles' ), RTEC_VERSION );
	}

	wp_enqueue_script( 'rtec_scripts', rtec_frontend_asset_url( 'assets/frontend/js/rtec-scripts.js' ), array( 'jquery' ), RTEC_VERSION, true );
