...
/**
 * Default/fallback meta for an event (used when the event no longer exists).
 *
 * @param int|string $id Event ID.
 *
 * @return array
 */
function rtec_get_default_event_meta( $id = 0 ) {
	$event_meta                          = array();
	$event_meta['post_id']               = $id;
	$event_meta['title']                 = __( '(Deleted event)', 'registrations-for-the-events-calendar' );
	$event_meta['start_date']            = '';
	$event_meta['end_date']              = '';
	$event_meta['start_date_utc']        = '';
	$event_meta['end_date_utc']          = '';
	$event_meta['venue_id']              = '';
	$event_meta['venue_title']           = '';
	$event_meta['venue_address']         = '';
	$event_meta['venue_city']            = '';
	$event_meta['venue_state']           = '';
	$event_meta['venue_zip']             = '';
	$event_meta['registrations_disabled'] = true;
	$event_meta['show_registrants_data'] = false;
	$event_meta['who_can_register']      = 'any';
	$event_meta['limit_registrations']   = false;
	$event_meta['max_registrations']     = 0;
	$event_meta['num_registered']        = 0;
	$event_meta['form_id']               = 0;
	$event_meta['registrations_left']    = '';
	$event_meta['open_type']             = 'immediate';
	$event_meta['open_offset']           = 0;
	$event_meta['open_other_timestamp']  = 0;
	$event_meta['deadline_type']         = 'start';
	$event_meta['deadline_other_timestamp'] = 0;
	$event_meta['registration_deadline'] = 0;
...
function rtec_get_event_meta( $id = '' ) {
	...
	// If the event does not exist (e.g., it was deleted), return a lightweight fallback
	// so callers (like the All Registrations view) can handle it gracefully without
	// triggering heavy lookups or fatal errors.
	if ( ! empty( $id ) && ! get_post( $id ) ) {
		return rtec_get_default_event_meta( $id );
	}
	...
	$default_open_type                  = isset( $rtec_options['default_open_type'] ) ? $rtec_options['default_open_type'] : 'immediate';
	$event_meta['open_type']            = isset( $meta['_RTECopenType'][0] ) ? $meta['_RTECopenType'][0] : $default_open_type;
	$event_meta['open_offset']          = isset( $meta['_RTECopenOffset'][0] ) ? $meta['_RTECopenOffset'][0] : 3;
	$event_meta['open_other_timestamp'] = isset( $meta['_RTECopenTimeStamp'][0] ) ? $meta['_RTECopenTimeStamp'][0] : strtotime( $event_meta['start_date'] );
...
function rtec_get_event_deadline_utc( $event_meta ) {
	...
	} elseif ( $event_meta['deadline_type'] === 'start' ) {
...
function rtec_format_phone_number( $raw_number ) {
	...
	if ( $phone_option === '2' ) {
		...
		$rules[10]['pattern']     = '/([0-9]{2})([0-9]{4})([0-9]{4})/';
		$rules[10]['replacement'] = '($1) $2 $3';
	} elseif ( $phone_option === '5' ) {
		$rules[10]['pattern']     = '/([0-9]{3})([0-9]{3})([0-9]{4})/';
		$rules[10]['replacement'] = '+1-$1-$2-$3';
	}
...
/**
 * For admin tables: truncate long form values and show full text in the cell on hover.
 *
 * @param string $text       Raw value to display.
 * @param int    $max_length Max characters before truncation. Default 50.
 * @return string HTML: escaped text, or truncated + full value (full shown inline on hover).
 */
function rtec_admin_truncate_with_tooltip( $text, $max_length = 100 ) {
	$text = (string) $text;
	if ( $text === '' ) {
		return '';
	}
	...
	$full_esc  = esc_html( $text );
	$short_esc = esc_html( $truncated ) . '…';
	return '<span class="rtec-truncate-wrap"><span class="rtec-truncate-visible">' . $short_esc . '</span><span class="rtec-truncate-full">' . $full_esc . '</span></span>';
}

/**
 * Returns HTML for a CSS-only hover tooltip (icon + bubble) for settings.
 * Use next to a label; the bubble appears above the icon on hover.
 *
 * @param string $description Help text to show in the tooltip. If empty, returns empty string.
 * @param bool   $right_aligned Optional. If true, bubble appears to the right of the icon. Default false.
 * @return string HTML for the tooltip wrapper, or empty string if $description is empty.
 */
function rtec_get_setting_tooltip_html( $description, $right_aligned = false ) {
	if ( $description === '' || $description === null ) {
		return '';
	}
	$bubble_class = 'rtec-tooltip-bubble rtec-tooltip-shadow';
	if ( $right_aligned ) {
		$bubble_class .= ' rtec-tooltip-right';
	}
	$info_icon = '<svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true"><circle cx="8" cy="8" r="8" fill="#D9D9D9"/><circle cx="8" cy="4" r="1" fill="#656565"/><rect x="7" y="7" width="2" height="6" fill="#656565"/></svg>';
	return sprintf(
		'<span class="rtec-tooltip-wrap"><a href="javascript:void(0);" class="rtec-tooltip-link" aria-label="%1$s">%2$s</a><span class="%3$s"><p>%4$s</p></span></span>',
		esc_attr__( 'More information', 'registrations-for-the-events-calendar' ),
		$info_icon,
		esc_attr( $bubble_class ),
...
function rtec_generate_unregister_link( $event_id, $action_key, $email, $unregister_link_text, $confirm_required = false ) {
	...
	if ( $confirm_required ) {
		$unregister_url = add_query_arg( 'confirm_required' , '1', $unregister_url );
	}
...
function rtec_generate_unregister_button( $event_id, $action_key, $email, $confirm_required = false ) {
	global $rtec_options;
	$permalink      = get_permalink( $event_id );
	$unregister_url = add_query_arg( 'action', 'unregister', $permalink );
	$unregister_url = add_query_arg( 'token', $action_key, $unregister_url );
	...
	$button_template        = $custom_button_template ? $custom_button_template : rtec_plugin_path( 'templates/email/button-generic.php' );
	...
	foreach ( $entry_ids as $entry_id ) {
		...
		if ( isset( $entries[0] ) ) {
			...
			if ( is_email( $email ) ) {
				...
				$subject    = ! empty( $rtec_options['unregister_notification_subject'] ) ? $rtec_options['unregister_notification_subject'] : __( 'Notification of Cancellation', 'registrations-for-the-events-calendar' ) . ' {event-title}';
				$subject    = rtec_get_text( $subject, __( 'Notification of Cancellation', 'registrations-for-the-events-calendar' ) );
 ...
 * @since 1.0
 */
function rtec_get_max_admin_last_viewed_registrations_at() {
	$cached = get_transient( 'rtec_max_admin_last_viewed_at' );
	...
	global $wpdb;
	$meta_key = RTEC_LAST_VIEWED_REGISTRATIONS_META_KEY;
	$max      = $wpdb->get_var(
		$wpdb->prepare(
			"SELECT MAX(CAST(um.meta_value AS UNSIGNED)) FROM {$wpdb->usermeta} um WHERE um.meta_key = %s",
			$meta_key
		)
	);
	$max = ( $max !== null && is_numeric( $max ) ) ? (int) $max : 0;
	set_transient( 'rtec_max_admin_last_viewed_at', $max, 10 * MINUTE_IN_SECONDS );
	return $max;
}
...
function rtec_tribe_get_option( $option_name ) {
	if ( is_callable( 'tribe_get_option' ) ) {
		return tribe_get_option( $option_name );
	}
...
/**
 * URL for the attendee list (dashboard), optionally for a specific event and/or registration.
 * Available on frontend and admin so notification emails can include the link.
 *
 * @since 3.0
 * @param int $event_id        Optional. Event post ID to filter to.
 * @param int $registration_id Optional. Registration record ID (adds &record= for deep linking).
 * @return string
 */
function rtec_get_attendees_dashboard_url( $event_id = 0, $registration_id = 0 ) {
	$slug = defined( 'RTEC_MENU_SLUG' ) ? RTEC_MENU_SLUG : 'registrations-for-the-events-calendar';
	if ( $event_id ) {
		$url = admin_url( 'admin.php?page=' . $slug . '&tab=single&id=' . (int) $event_id );
		if ( $registration_id ) {
			$url = add_query_arg( 'record', (int) $registration_id, $url );
		}
		return $url;
	}
	return admin_url( 'admin.php?page=' . $slug . '&tab=overview' );
}