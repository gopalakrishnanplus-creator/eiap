...
class RTEC_Email {
	...
	public function get_generic_confirmation( $sanitized_data ) {
		...
		$body 		= __( 'Hey {first},<br /><br />You are registered for {event-title} at {venue} on {event-date}. We are looking forward to having you there. The event will be held at this location:<br /><br />{venue-address}<br />{venue-city}, {venue-state} {venue-zip}<br /><br />See you there!', 'registrations-for-the-events-calendar' );
	...
	public function get_generic_submission_notification( $sanitized_data ) {
		...
		$body .= '<p>{admin-view-in-dashboard}</p>';
	...
	protected function set_message_body( $args ) {
		...
		if ( ! empty( $args['message'] ) && ! empty( $args['data'] ) ) {
			$body = wp_unslash( $this->find_and_replace( $args['message'], $args['data'] ) );
		} else {
			$body = wp_unslash( $args['message'] );
		}
		...
		if ( $this->content_type === 'html' ) {
			...
			$header_template        = $custom_header_template ? $custom_header_template : rtec_plugin_path( 'templates/email/header-generic.php' );
			include $header_template;

			$custom_confirmation_template = locate_template( 'rtec/email/confirmation-body.php', false, false );
			$confirmation_template        = $custom_confirmation_template ? $custom_confirmation_template : rtec_plugin_path( 'templates/email/confirmation-body.php' );
			include $confirmation_template;

			$custom_footer_template = locate_template( 'rtec/email/footer-generic.php', false, false );
			$footer_template        = $custom_footer_template ? $custom_footer_template : rtec_plugin_path( 'templates/email/footer-generic.php' );
	...
	public static function event_summary_placeholder( $sanitized_data ) {
		$event_id = isset( $sanitized_data['event_id'] ) ? $sanitized_data['event_id'] : 0;

		$body = '<table><tbody>';

		$body .= '<tr>';
		$body .= sprintf( '<td>%s&#58;</td><td>%s</td>', esc_html__( 'Title', 'registrations-for-the-events-calendar' ), esc_html( get_the_title( $event_id ) ) );
		$body .= '</tr>';

		$body .= '<tr>';
		$body .= sprintf( '<td>%s&#58;</td><td>%s</td>', esc_html__( 'Date', 'registrations-for-the-events-calendar' ), esc_html( tribe_get_start_date( $event_id ) ) );
		$body .= '</tr>';

		$body .= '<tr>';
		$body .= sprintf( '<td>%s&#58;</td><td>%s</td>', esc_html__( 'Link', 'registrations-for-the-events-calendar' ), get_the_permalink( $event_id ) );
		$body .= '</tr>';
	...
	protected function find_and_replace( $text, $sanitized_data ) {
		...
		if ( ( strpos( $text, '{all-fields}' ) ) !== false && ! empty( $sanitized_data['event_id'] ) ) {
			$all_fields = self::all_fields_placeholder( $sanitized_data );
		}

		$event_summary = '';
		if ( ( strpos( $text, '{event-summary}' ) ) !== false && ! empty( $sanitized_data['event_id'] ) ) {
			$event_summary = self::event_summary_placeholder( $sanitized_data );
		}
		...
		if ( ! empty( $sanitized_data ) ) {
				...
				foreach ( $this->custom_template_pairs  as $field => $atts ) {
					...
					if ( $atts['label'] !== '' ) {
						...
						if ( isset( $sanitized_data['custom'][ $field ] ) ) {
							$search_replace[ '{' . str_replace( '&#42;', '', wp_unslash( $atts['label'] ) ) . '}' ] = $sanitized_data['custom'][ $field ]['value'];
						} elseif ( isset( $sanitized_data['custom'][ $atts['label'] ] ) ) {
							$search_replace[ '{' . str_replace( '&#42;', '', wp_unslash( $atts['label'] ) ) . '}' ] = $sanitized_data['custom'][ $atts['label'] ];
						} elseif ( isset( $sanitized_data[ $field ] ) ) {
							$search_replace[ '{' . str_replace( '&#42;', '', wp_unslash( $atts['label'] ) ) . '}' ] = $sanitized_data[ $field ];
						} else {
							$search_replace[ '{' . str_replace( '&#42;', '', wp_unslash( $atts['label'] ) ) . '}' ] = '';
						}
					}
				}
			}
			...
			$has_cancel_placeholder = ( strpos( $working_text, '{cancel-link}' ) !== false || strpos( $working_text, '{unregister-link}' ) !== false );
			if ( $has_cancel_placeholder ) {
				if ( isset( $sanitized_data['action_key'] ) && $sanitized_data['action_key'] != '' ) {
					$unregister_link_text = isset( $rtec_options['unregister_link_text'] ) ? $rtec_options['unregister_link_text'] : __( 'Cancel my registration', 'registrations-for-the-events-calendar' );
				...
				$working_text = str_replace( array( '{cancel-link}', '{unregister-link}' ), $u_link, $working_text );
			}

			$has_cancel_button = ( strpos( $working_text, '{cancel-button}' ) !== false || strpos( $working_text, '{unregister-button}' ) !== false );
			if ( $has_cancel_button ) {
				$u_button = rtec_generate_unregister_button( $sanitized_data['event_id'], $sanitized_data['action_key'], $sanitized_data['email'], $require_confirmation );
				$working_text = str_replace( array( '{cancel-button}', '{unregister-button}' ), $u_button, $working_text );
			}

			// Notification-only placeholder: link to view registration in dashboard. In confirmation or elsewhere, replace with nothing.
			if ( strpos( $working_text, '{admin-view-in-dashboard}' ) !== false ) {
				if ( $this->template_type === 'notification' && function_exists( 'rtec_get_attendees_dashboard_url' ) ) {
					$event_id        = isset( $sanitized_data['event_id'] ) ? absint( $sanitized_data['event_id'] ) : 0;
					$registration_id = isset( $sanitized_data['registration_id'] ) ? absint( $sanitized_data['registration_id'] ) : 0;
					$dashboard_url   = rtec_get_attendees_dashboard_url( $event_id, $registration_id );
					$link_text       = isset( $rtec_options['admin_view_in_dashboard_text'] ) ? $rtec_options['admin_view_in_dashboard_text'] : __( 'View registration in dashboard', 'registrations-for-the-events-calendar' );
					$link_text       = rtec_get_text( $link_text, __( 'View registration in dashboard', 'registrations-for-the-events-calendar' ) );
					$admin_link      = ( $event_id && $dashboard_url ) ? '<a href="' . esc_url( $dashboard_url ) . '">' . esc_html( $link_text ) . '</a>' : '';
					$working_text    = str_replace( '{admin-view-in-dashboard}', $admin_link, $working_text );
				} else {
					$working_text = str_replace( '{admin-view-in-dashboard}', '', $working_text );
				}
			}
		}
