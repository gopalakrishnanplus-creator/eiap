<?php
/**
 * New registrations count (created after current user last viewed Registrations area).
 * Delegates to RTEC_New_Registration_Alerts_Service.
 *
 * @return int Count of new registrations; 0 if none or never viewed.
 * @since 1.0
 */
function rtec_get_existing_new_reg_count() {
	if ( ! class_exists( 'RTEC_New_Registration_Alerts_Service' ) ) {
		return 0;
	}
	return RTEC_New_Registration_Alerts_Service::instance()->get_new_count();
}

/**
 * Updates the current user's "last viewed registrations" timestamp.
 * Call when the admin views the All Registrations tab or a single event's registrations tab.
 *
 * @since 1.0
 */
function rtec_update_admin_last_viewed_registrations() {
	if ( ! class_exists( 'RTEC_New_Registration_Alerts_Service' ) ) {
		return;
	}
	RTEC_New_Registration_Alerts_Service::instance()->update_last_viewed();
}

/**
 * Redirect legacy Events-based Registrations URLs to the main admin page.
 * Ensures edit.php?post_type=tribe_events&page=... never loads; all traffic goes to admin.php?page=registrations-for-the-events-calendar.
 *
 * @since 1.0
 */
function rtec_redirect_legacy_events_registrations_url() {
	...
	$page_decoded = rawurldecode( $page );
	$is_legacy    = ( $post_type === 'tribe_events' )
		&& ( $page === 'registrations-for-the-events-calendar'
			|| strpos( $page_decoded, 'registrations-for-the-events-calendar' ) === 0 );

	if ( ! $is_legacy ) {
		return;
	}

	$redirect = admin_url( 'admin.php?page=registrations-for-the-events-calendar' );
	$preserve = array( 'tab', 'id', 'v', 'qtype', 'with', 'off', 'start', 'record', 'mvt', 'rtec_troubleshoot' );
	foreach ( $preserve as $key ) {
		if ( isset( $_GET[ $key ] ) && $_GET[ $key ] !== '' ) {
			$redirect = add_query_arg( $key, sanitize_text_field( wp_unslash( $_GET[ $key ] ) ), $redirect );
		}
	}
	wp_safe_redirect( $redirect );
	exit;
}
	add_action( 'admin_init', 'rtec_redirect_legacy_events_registrations_url', 1 );

/**
 * Redirect legacy settings page URLs to the unified Settings page with tab.
 * rtec-form -> rtec-settings&tab=form, rtec-email -> rtec-settings&tab=email, rtec-support -> rtec-settings&tab=support.
 *
 * @since 1.0
 */
function rtec_redirect_legacy_settings_urls() {
	if ( ! is_admin() || wp_doing_ajax() || wp_doing_cron() ) {
		return;
	}
	...
	$redirect = admin_url( 'admin.php?page=rtec-settings&tab=' . $map[ $page ] );
	wp_safe_redirect( $redirect );
	exit;
}
add_action( 'admin_init', 'rtec_redirect_legacy_settings_urls', 2 );
...
function rtec_meta_boxes_html() {
    ...
    <div class="rtec-series-flag rtec-notice rtec-box-shadow rtec-is-warning">
        <?php echo RTEC_Icon::get( 'warning' ); ?>
	...
	$db = new RTEC_Db_Admin();
		...
		switch ( $action ) {
			case 'delete':
				...
				if ( ! empty( $_POST['registrations_to_be_deleted'] ) && is_array( $_POST['registrations_to_be_deleted'] ) ) {
		...
		foreach ( $event_obj->registrants_data as $registration ) {
			...
			foreach ( $event_obj->column_label as $column => $label ) {
				...
				if ( isset( $registration[ $column ] ) ) {
					$value = wp_unslash( $registration[ $column ] );
				} elseif ( isset( $registration[ $column . '_name' ] ) ) {
					$value = wp_unslash( $registration[ $column . '_name' ] );
				} elseif ( isset( $registration['custom'][ $column ] ) ) {
					$value = wp_unslash( $registration['custom'][ $column ]['value'] );
				} elseif ( isset( $registration['custom'][ $label ] ) ) {
					$value = wp_unslash( $registration['custom'][ $label ] );
				}
		...
		if ( ! empty( $matches ) ) :
			...
			">
				<td class="rtec-first-data">
					...
					if ( $is_new ) {
						echo '<span class="rtec-new-registration-tag rtec-tooltip-wrap"><span class="rtec-icon-tag rtec-icon">' . RTEC_Icon::get( 'tag' ) . '</span><span class="rtec-status-tooltip rtec-tooltip-shadow"><span class="rtec-status-tooltip-text">' . esc_html__( 'New registration', 'registrations-for-the-events-calendar' ) . '</span></span></span> ';
					}
					...
					<td>
								...
								if ( isset( $registration[ $column ] ) ) {
									...
									if ( $column === 'phone' ) {
										echo esc_html( rtec_format_phone_number( wp_unslash( $registration[ $column ] ) ) );
									} elseif ( $column === 'email' ) {
				...
				<?php endforeach; ?>
				<td><a href="<?php echo esc_url( admin_url( RTEC_ADMIN_URL . '&tab=single&id=' . (int) $event_meta['post_id'] . '&record=' . (int) $registration['id'] . '&mvt=' ) ); ?>"><?php echo esc_html( $event_meta['title'] ); ?></a></td>
...
function rtec_admin_scripts_and_styles() {
	...
	$evge_screen_page = isset( $_GET['page'] ) ? sanitize_text_field( wp_unslash( $_GET['page'] ) ) : '';
	...
	if ( ! class_exists( 'Tribe__Main' ) ) {
		wp_enqueue_script( 'rtec_installer_script', rtec_plugin_url( 'assets/admin/js/rtec-installer.js' ), array( 'jquery' ), RTEC_VERSION, true );
		$data    = array(
		...
		wp_localize_script(
			...
			wp_enqueue_script( 'rtec_admin_scripts', rtec_plugin_url( 'assets/admin/js/rtec-admin-scripts.js' ), array( 'jquery', 'jquery-ui-datepicker', 'tribe-jquery-timepicker' ), RTEC_VERSION, false );
			wp_localize_script(
				'rtec_admin_scripts',
				'rtecAdminScript',
				array(
					'ajax_url'             => admin_url( 'admin-ajax.php' ),
					'rtec_nonce'           => wp_create_nonce( 'rtec_nonce' ),
					'toggle_show_details'  => __( 'Show details', 'registrations-for-the-events-calendar' ),
					'toggle_hide_details'  => __( 'Hide details', 'registrations-for-the-events-calendar' ),
				)
			);
			wp_enqueue_script( 'rtec_modal_content', rtec_plugin_url( 'assets/admin/js/rtec-modal-content.js' ), array( 'jquery', 'rtec_admin_scripts' ), RTEC_VERSION, true );
			wp_enqueue_style( 'rtec_content_modal', rtec_plugin_url( 'assets/admin/css/rtec-content-modal.css' ), array( 'rtec_admin_styles' ), RTEC_VERSION );
			wp_enqueue_script( 'rtec_placeholder_reference', rtec_plugin_url( 'assets/admin/js/rtec-placeholder-reference.js' ), array( 'jquery' ), RTEC_VERSION, true );
			wp_enqueue_style( 'rtec_placeholder_reference', rtec_plugin_url( 'assets/admin/css/rtec-placeholder-reference.css' ), array(), RTEC_VERSION );
			wp_enqueue_script( 'rtec_settings_checklist', rtec_plugin_url( 'assets/admin/js/rtec-settings-checklist.js' ), array(), RTEC_VERSION, true );
		...
		wp_enqueue_script( 'rtec_admin_edit_event_scripts', rtec_plugin_url( 'assets/admin/js/rtec-admin-edit-event-scripts.js' ), array( 'jquery' ), RTEC_VERSION, false );
		wp_enqueue_script( 'jquery-ui-datepicker' );
		wp_enqueue_style( 'jquery-ui-smoothness', 'https://code.jquery.com/ui/1.12.1/themes/smoothness/jquery-ui.css', array(), '1.12.1' );
		wp_enqueue_script( 'tribe-jquery-timepicker' );
		wp_enqueue_style( 'tribe-jquery-timepicker-css' );
...
function rtec_plugin_action_links( $links ) {
	...
	$settings_link = '<a href="' . esc_url( admin_url( 'admin.php?page=rtec-settings&tab=form' ) ) . '">' . __( 'Settings', 'registrations-for-the-events-calendar' ) . '</a>';
...
/**
 * Whether an SMTP plugin is detected (common ones).
 *
 * @since 3.0
 * @return bool
 */
function rtec_checklist_smtp_detected() {
	$smtp_plugins = array(
		...
		'mailjet-for-wordpress/mailjet-for-wordpress.php',
...
/**
 * Whether the "from" email domain matches the site domain (rough deliverability hint).
 *
 * @since 3.0
 * @return bool
 */
function rtec_checklist_from_domain_matches_site() {
	global $rtec_options;
	$rtec_options = isset( $rtec_options ) ? $rtec_options : get_option( 'rtec_options', array() );
	$from         = isset( $rtec_options['confirmation_from_address'] ) ? $rtec_options['confirmation_from_address'] : '';
	if ( ! is_email( $from ) ) {
		return false;
	}
	$from_domain = strtolower( substr( strrchr( $from, '@' ), 1 ) );
	$site_host   = isset( $_SERVER['HTTP_HOST'] ) ? strtolower( sanitize_text_field( wp_unslash( $_SERVER['HTTP_HOST'] ) ) ) : '';
	$site_host   = preg_replace( '/^www\./', '', $site_host );
	return ( $from_domain === $site_host || strpos( $site_host, $from_domain ) !== false );
}
...
function rtec_db_update_check() {
	...
	if ( $db_ver < 1.7 ) {
		update_option( 'rtec_db_version', RTEC_DBVERSION );
		$onb_raw = get_option( 'rtec_onboarding_state', null );
		if ( is_array( $onb_raw ) && ! empty( $onb_raw ) && ! array_key_exists( 'checklist_initiated', $onb_raw ) ) {
			$onb_raw['checklist_initiated'] = true;
			update_option( 'rtec_onboarding_state', $onb_raw );
		}
	}
