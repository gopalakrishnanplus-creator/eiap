...
class RTEC_Admin_Registrations {
	...
	public function get_events( $full = false ) {
		...
		if ( $settings['qtype'] === 'all' ) {
			...
			if ( ! empty( $this->settings['event_search'] ) ) {
				$args['s'] = $this->settings['event_search'];
			}
			...
			if ( $this->settings['with'] === 'with' ) {
				$args['meta_query'] = array(
					...
					array(
						...
						array(
							'key'     => '_EventEndDate',
							'value'   => date( 'Y-m-d H:i', time() - ( 1 * DAY_IN_SECONDS ) + rtec_get_utc_offset() ),
							'compare' => '>=',
							'type'    => 'DATE',
						),
					),
	...
	public function the_toolbar() {
		require_once rtec_plugin_path( 'admin-templates/partials/registrations-toolbar.php' );
	}
	...
	public function the_events_list() {
		require_once rtec_plugin_path( 'admin-templates/partials/registrations-list-view.php' );
	}
	...
	public function the_events_list_table_body() {
		...
		foreach ( $events as $event ) {
			if ( function_exists( 'tribe_is_event' ) && ! tribe_is_event( $event->ID ) ) {
				continue;
			}
			...
			if ( rtec_should_show( $settings['with'], $event_meta['registrations_disabled'] ) ) {
				include rtec_plugin_path( 'admin-templates/partials/registrations-list-table-body.php' );
			}
		}
	}
	...
	public function the_events_overview() {
		...
		if ( ! empty( $events ) ) {
			echo '<div class="rtec-items-grid rtec-events-overview-grid">';
			foreach ( $events as $event ) {
				$event_meta = rtec_get_event_meta( $event->ID );
				$this->add_event_id_on_page( $event->ID );

				if ( rtec_should_show( $settings['with'], $event_meta['registrations_disabled'] ) ) {
					echo '<div class="rtec-events-grid-item">';
					$event_obj = new RTEC_Admin_Event();
					...
					include rtec_plugin_path( 'admin-templates/partials/registrations-overview-view.php' );
					if ( ! empty( $event_obj->mvt_fields ) ) {
						echo '</div>';
					}
					echo '</div>';
	...
	public function the_event_meta( $event_obj ) {
		include rtec_plugin_path( 'admin-templates/partials/registrations-event-meta.php' );
	}
	...
	public function the_hidden_event_options( $event_obj ) {
		// include rtec_plugin_path( 'admin-templates/partials/registrations-hidden-event-options.php' );
	}
	...
	public function the_pagination() {
		require_once rtec_plugin_path( 'admin-templates/partials/registrations-pagination.php' );
	}
	...
	/**
	 * Returns the URL for the detailed (single event) registration view.
	 *
	 * @param int    $id  Event post ID.
	 * @param string $mvt Optional MVT id (unused in free version).
	 * @return string
	 */
	public function get_detailed_view_url( $id, $mvt = '' ) {
		$href            = admin_url( 'admin.php?page=registrations-for-the-events-calendar' );
		$settings        = $this->settings;
		$settings['tab'] = 'single';
		$settings['id']  = $id;
		...
		return add_query_arg( $query_args_array, $href );
	}
	...
	public function the_detailed_view_href( $id, $mvt = '' ) {
		echo esc_url( $this->get_detailed_view_url( $id, $mvt ) );
	}
	...
	public function get_registrant_icons( $status = 'c', $is_user = false ) {
		...
		switch ( $status ) {
			case 'c':
				...
				break;
			case 'p':
				$html .= '<span class="rtec-notice-new rtec-unconfirmed"><span class="rtec-icon" aria-hidden="true">' . RTEC_Icon::get( 'flag' ) . '</span></span>';
				break;
			default:
				$html .= '';
		}
		...
		if ( $is_user ) {
			...
			$html .= '<span class="rtec-notice-new rtec-is-user"><span class="rtec-icon" aria-hidden="true">' . RTEC_Icon::get( 'user' ) . '</span></span>';
