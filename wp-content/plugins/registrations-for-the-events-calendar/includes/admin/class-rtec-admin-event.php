	...
	public function get_registrations( $type = 'simple' ) {
		...
		if ( $type === 'simple' ) {
			...
			foreach ( $field_atts as $field => $atts ) {
				if ( ! in_array( $field, $no_backend_label_fields, true ) ) {
					$columns[]              = $field;
					$labels[]               = str_replace( '&#42;', '', __( wp_unslash( $atts['label'] ), 'registrations-for-the-events-calendar' ) );
					$column_label[ $field ] = str_replace( '&#42;', '', wp_unslash( $atts['label'] ) );
				}
			}
		}
