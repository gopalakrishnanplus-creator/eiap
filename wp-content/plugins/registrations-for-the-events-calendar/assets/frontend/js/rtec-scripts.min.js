...
jQuery(document).ready(function($){var SPINNER_HTML='<div class="rtec-spinner-container"><div class="rtec-spinner-circle"></div></div>';var RtecCommon={spinnerHTML:function(){return SPINNER_HTML},startProcessing:function($target,context,before){if(before===undefined){before=!1}
if(context==='button'){$target.attr('aria-busy','true');$target.wrap('<div class="rtec-processing-wrap-flex rtec-is-processing"></div>');$target.addClass('rtec-fade').prop('disabled',!0);if(before){$target.before(SPINNER_HTML)}else{$target.after(SPINNER_HTML)}}else if(context==='area'){$target.wrap('<div class="rtec-processing-wrap rtec-is-processing"></div>');$target.addClass('rtec-fade');$target.after(SPINNER_HTML)}else if(context==='element'){$target.addClass('rtec-fade rtec-is-processing');$target.append(SPINNER_HTML)}},stopProcessing:function($target,context){setTimeout(function(){if(context==='button'){$target.closest('.rtec-processing-wrap-flex').find('.rtec-spinner-container').remove();$target.unwrap('.rtec-processing-wrap-flex');$target.removeClass('rtec-fade').prop('disabled',!1).attr('aria-busy','false')}else if(context==='area'){$target.closest('.rtec-processing-wrap').find('.rtec-spinner-container').remove();$target.removeClass('rtec-fade');$target.unwrap('.rtec-processing-wrap')}else if(context==='element'){$target.find('.rtec-spinner-container').remove();$target.removeClass('rtec-fade rtec-is-processing')}},500)},showLoading:function($element){$element.addClass('rtec-is-processing');if($element.find('.rtec-spinner-container').length===0){$element.append(SPINNER_HTML)}},hideLoading:function($element){$element.removeClass('rtec-is-processing rtec-fade');$element.find('.rtec-spinner-container').fadeOut(200,function(){$(this).remove()})}};window.RtecCommon=RtecCommon;function rtecHandleServerError($context,opts){opts=opts||{};var msg=(typeof rtec!=='undefined'&&rtec.translations&&rtec.translations.serverError)?rtec.translations.serverError:'Something went wrong. Please try again.';var $notice=$('<div class="rtec-attendance tribe-events-notices rtec-warning" role="alert"><strong>'+msg+'</strong></div>');if($context&&$context.length){$context.find('.rtec-attendance.rtec-warning').remove();$context.prepend($notice)}else if($('.rtec').length){$('.rtec').first().find('.rtec-attendance.rtec-warning').remove();$('.rtec').first().prepend($notice)}
if(opts.stopProcessing&&opts.stopProcessing.length&&window.RtecCommon){window.RtecCommon.stopProcessing(opts.stopProcessing,opts.processingContext||'area')}
if(opts.enableButtons&&opts.enableButtons.length){opts.enableButtons.prop('disabled',!1).css('opacity',1)}}
if(typeof window.rtecInit==='undefined'){window.rtecInit=function(){$('.rtec').addClass('rtec-initialized');$('.rtec-js-show').show();$('.rtec-js-hide').hide();if($('#rtec-js-move-flag').length||$('.rtec-js-placement').length||$('footer').find('.rtec').length||$('footer').find('.rtec-outer-wrap').length||$('footer').find('.rtec-success-message').length){var $moveEl=$('#rtec-js-move-flag'),rtecLocation=typeof $('.rtec-outer-wrap').attr('data-location')!=='undefined'?$('.rtec-outer-wrap').attr('data-location'):!1;if(!rtecLocation){rtecLocation=typeof $('#rtec-js-move-flag').attr('data-location')!=='undefined'?$('#rtec-js-move-flag').attr('data-location'):'tribe_events_single_event_before_the_content'}
...
$form.find('input[type=submit]').prop('disabled',!0).css('opacity',.1);var submittedData={'action':action,'event_id':$outerWrap.find('.rtec').attr('data-event'),'email':$form.find('input[name=rtec-visitor_email]').val()};$.ajax({url:rtec.ajaxUrl,type:'post',data:submittedData,success:function(data){if(window.RtecCommon){var $processingWrap=$form.closest('.rtec-processing-wrap');$form.removeClass('rtec-fade');window.RtecCommon.stopProcessing($processingWrap.length?$processingWrap:$form,'area')}
$form.find('input[type=submit]').prop('disabled',!1).css('opacity',1);if(data.trim().indexOf('{')>-1){var response=JSON.parse(data.trim());if(typeof response.success!=='undefined'){$form.replaceWith(response.success)}else if(typeof response.error!=='undefined'){var $formField=$form.find('input[name=rtec-visitor_email]').closest('.rtec-input-wrapper');if(!$formField.find('.rtec-error-message').length){$formField.append('<p class="rtec-error-message" role="alert">'+response.error+'</p>')}
$form.find('input[name=rtec-visitor_email]').attr('aria-invalid','true')}}},error:function(){rtecHandleServerError($outerWrap.length?$outerWrap:$form.closest('.rtec'),{stopProcessing:$form,processingContext:'area',enableButtons:$form.find('input[type=submit]')})}})});$form.find('input[type=submit]').on('click',function(){$("input[type=submit]",$(this).parents("form")).prop("clicked",!1);$(this).prop("clicked","true")})}
$rtec.find('.rtec-unregister-link').on('click',function(event){event.preventDefault();var $link=$(this);if($link.hasClass('rtec-processing')){return}
$link.addClass('rtec-processing');var $wrapEl=$link.closest('.rtec-already-registered-options'),$rtecWrap=$wrapEl.closest('.rtec-outer-wrap'),submittedData={'event_id':$link.attr('data-event-id'),'action':'rtec_unregister_by_event_id_for_logged_in_user'};if(rtec.loggedInUnregisterNonce){submittedData.nonce=rtec.loggedInUnregisterNonce}
if(!$wrapEl.closest('.rtec-outer-wrap').length){$rtecWrap=$('.rtec').closest('div')}
if(window.RtecCommon){window.RtecCommon.showLoading($wrapEl)}
$.ajax({url:rtec.ajaxUrl,type:'post',data:submittedData,success:function(data){var response=data;if(typeof data==='string'){try{response=JSON.parse(data)}catch(e){console.error('Failed to parse JSON response:',e);return}}
...
formEl.addClass(RtecForm.validClass);RtecForm.removeErrorMessage(formEl)}else{if(formEl.hasClass(RtecForm.validClass)){formEl.removeClass(RtecForm.validClass)}
formEl.addClass(RtecForm.invalidClass);RtecForm.showErrorMessage(formEl)}},enableSubmitButton:function(_callback,$context){if(_callback()){$context.find('input[name=rtec_submit]').prop('disabled',!1).css('opacity',1)}},isDuplicateEmail:function(email,eventID,$context){var $emailEl=$context.find('input[name=rtec_email]'),spinnerHtml=(window.RtecCommon&&window.RtecCommon.spinnerHTML)?window.RtecCommon.spinnerHTML():'',$spinner='<span class="rtec-email-spinner">'+spinnerHtml+'</span>';$emailEl.closest('div').append($spinner);var submittedData={'action':'rtec_registrant_check_for_duplicate_email','event_id':eventID,'email':email.trim()};$.ajax({url:rtec.ajaxUrl,type:'post',data:submittedData,success:function(data){var json={'approved':!0,'message':''};if(data.trim().indexOf('{')===0){json=JSON.parse(data)}else if(data.indexOf('<p class=')>-1){json={'approved':!1,'message':data}}
...
$context.find('input[name=rtec_submit]').prop('disabled',!1).css('opacity',1);$context.find('.rtec-email-spinner').remove()},error:function(){$context.find('input[name=rtec_submit]').prop('disabled',!1).css('opacity',1);$context.find('.rtec-email-spinner').remove()}})}};if(typeof rtec.checkForDuplicates!=='undefined'&&rtec.checkForDuplicates==='1'){var $rtecEmailField=$('input[name=rtec_email]'),typingTimer,doneTypingInterval=1500;$rtecEmailField.on('input',function(){var $this=$(this),$context=$this.closest('.rtec');$context.find('input[name=rtec_submit]').prop('disabled',!0).css('opacity','.5');clearTimeout(typingTimer);typingTimer=setTimeout(function(){var $eventID=$context.find('input[name=rtec_event_id]').val();if(RtecForm.isValidEmail($this.val())){RtecForm.isDuplicateEmail($this.val(),$eventID,$context)}},doneTypingInterval)});$rtecEmailField.each(function(){var $this=$(this),$context=$this.closest('.rtec'),$eventID=$context.find('input[name=rtec_event_id]').val();if(RtecForm.isValidEmail($this.val())){RtecForm.isDuplicateEmail($this.val(),$eventID,$context)}})}
...
function rtecSetToPending($elements){if(window.RtecCommon){if(Array.isArray($elements)){$elements.forEach(function($element){window.RtecCommon.showLoading($element)})}else{window.RtecCommon.showLoading($elements)}}}
function rtecSetToReady($elements){if(window.RtecCommon){if(Array.isArray($elements)){$elements.forEach(function($element){window.RtecCommon.hideLoading($element)})}else{window.RtecCommon.hideLoading($elements)}}}
$('#rtec-confirm-unregister-form').on('submit',function(event){event.preventDefault();var $form=$(this);if(window.RtecCommon){window.RtecCommon.startProcessing($form,'area')}
$form.find('button').prop('disabled',!0).css('opacity',.1);var submittedData={'action':'rtec_confirm_unregistration','event_id':$form.find('input[name=event_id]').val(),'rtec_unregister_confirm':$form.find('input[name=rtec_unregister_confirm]').val()};$.ajax({url:rtec.ajaxUrl,type:'post',data:submittedData,success:function(data){if(window.RtecCommon){window.RtecCommon.stopProcessing($form,'area')}
$form.find('button').prop('disabled',!1).css('opacity',1);if(data.trim().indexOf('<')>-1){$form.empty().append(data);$('#rtec-confirm-unregister').find('.rtec-attendance').fadeIn()}
var eventId=$form.find('input[name=event_id]').val();var $wrap=$form.closest('.rtec-outer-wrap').length?$form.closest('.rtec-outer-wrap'):$('.rtec-outer-wrap').first();if(eventId&&$wrap.length){var evt=$.Event('rtecregistrationchange');evt.el=$wrap;evt.eventId=eventId;$(window).trigger(evt)}},error:function(){if(window.RtecCommon){window.RtecCommon.stopProcessing($form,'area')}
$form.find('button').prop('disabled',!1).css('opacity',1);rtecHandleServerError($form.closest('#rtec-confirm-unregister').length?$form.closest('#rtec-confirm-unregister'):$('.rtec').first())}})});function rtecRefreshOnRegistrationChange(event){var $rtecEl=event.el&&event.el.closest('.rtec-outer-wrap').length?event.el.closest('.rtec-outer-wrap'):(event.el||$());var eventId=(event.eventId!=null&&event.eventId!=='')?String(event.eventId):(typeof $rtecEl.find('.rtec').attr('data-event')!=='undefined'?$rtecEl.find('.rtec').attr('data-event'):(event.el&&event.el.attr('data-event')));if(!eventId){return}
if(!rtecDynamicElementsExist($rtecEl)){return}
dynamicElements=rtecGetDynamicElements($rtecEl);rtecSetToPending(dynamicElements);$.ajax({url:rtec.ajaxUrl,type:'post',data:{'action':'rtec_refresh_event_info','event_id':eventId},success:function(data){rtecSetToReady(dynamicElements);var response=data;if(typeof data==='string'){try{response=JSON.parse(data)}catch(e){console.error('Failed to parse JSON response:',e);return}}
...
$(window).on('rtecsubmissionajax',function(event){var $rtecEl=event.el.closest('.rtec-outer-wrap').length?event.el.closest('.rtec-outer-wrap'):event.el;var eventId=typeof $rtecEl.attr('data-event')!=='undefined'?$rtecEl.attr('data-event'):event.el.attr('data-event');if(eventId&&$rtecEl.length){var changeEvt=$.Event('rtecregistrationchange');changeEvt.el=$rtecEl;changeEvt.eventId=eventId;$(window).trigger(changeEvt)}});$(window).on('rtecregistrationchange',rtecRefreshOnRegistrationChange)}}
