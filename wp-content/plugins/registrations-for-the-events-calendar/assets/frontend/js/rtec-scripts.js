...
jQuery(document).ready(function($) {
    // -------------------------------------------------------------------------
    // RtecCommon: processing state and loading spinner (aligned with Pro).
    // -------------------------------------------------------------------------
    ...
    var RtecCommon = {
        ...
        startProcessing: function($target, context, before) {
            if (before === undefined) {
                before = false;
            }
            if (context === 'button') {
                $target.attr('aria-busy', 'true');
                $target.wrap('<div class="rtec-processing-wrap-flex rtec-is-processing"></div>');
                $target.addClass('rtec-fade').prop('disabled', true);
                if (before) {
                    $target.before(SPINNER_HTML);
                } else {
                    $target.after(SPINNER_HTML);
                }
            } else if (context === 'area') {
                $target.wrap('<div class="rtec-processing-wrap rtec-is-processing"></div>');
                $target.addClass('rtec-fade');
                $target.after(SPINNER_HTML);
            } else if (context === 'element') {
                $target.addClass('rtec-fade rtec-is-processing');
                $target.append(SPINNER_HTML);
            }
        },
        stopProcessing: function($target, context) {
            setTimeout(function() {
                if (context === 'button') {
                    $target.closest('.rtec-processing-wrap-flex').find('.rtec-spinner-container').remove();
                    $target.unwrap('.rtec-processing-wrap-flex');
                    $target.removeClass('rtec-fade').prop('disabled', false).attr('aria-busy', 'false');
                } else if (context === 'area') {
                    $target.closest('.rtec-processing-wrap').find('.rtec-spinner-container').remove();
                    $target.removeClass('rtec-fade');
                    $target.unwrap('.rtec-processing-wrap');
                } else if (context === 'element') {
                    $target.find('.rtec-spinner-container').remove();
                    $target.removeClass('rtec-fade rtec-is-processing');
                }
            }, 500);
        },
        showLoading: function($element) {
            $element.addClass('rtec-is-processing');
            if ($element.find('.rtec-spinner-container').length === 0) {
                $element.append(SPINNER_HTML);
            }
        },
        hideLoading: function($element) {
            $element.removeClass('rtec-is-processing rtec-fade');
            $element.find('.rtec-spinner-container').fadeOut(200, function() {
                $(this).remove();
            });
        }
    };
    window.RtecCommon = RtecCommon;

    function rtecHandleServerError($context, opts) {
        opts = opts || {};
        var msg = (typeof rtec !== 'undefined' && rtec.translations && rtec.translations.serverError) ? rtec.translations.serverError : 'Something went wrong. Please try again.';
        var $notice = $('<div class="rtec-attendance tribe-events-notices rtec-warning" role="alert"><strong>' + msg + '</strong></div>');
        if ($context && $context.length) {
            $context.find('.rtec-attendance.rtec-warning').remove();
            $context.prepend($notice);
        } else if ($('.rtec').length) {
            $('.rtec').first().find('.rtec-attendance.rtec-warning').remove();
            $('.rtec').first().prepend($notice);
        }
        if (opts.stopProcessing && opts.stopProcessing.length && window.RtecCommon) {
            window.RtecCommon.stopProcessing(opts.stopProcessing, opts.processingContext || 'area');
        }
        if (opts.enableButtons && opts.enableButtons.length) {
            opts.enableButtons.prop('disabled', false).css('opacity', 1);
        }
    }
        ...
        window.rtecInit = function () {
            ...
            $('.rtec').each(function(index) {
                    ...
                    $form.on('submit',function(event) {
                        ...
                        $.ajax({
                            ...
                            success: function (data) {
                                // `stopProcessing()` is delayed; the success path may replace `$form`,
                                // so clean up using the processing wrapper instead of the detached form.
                                if (window.RtecCommon) {
                                    var $processingWrap = $form.closest('.rtec-processing-wrap');
                                    // Clear the dimming state immediately (even if the wrapper cleanup is delayed).
                                    $form.removeClass('rtec-fade');
                                    window.RtecCommon.stopProcessing($processingWrap.length ? $processingWrap : $form, 'area');
                                }
                            ...
                            error: function() {
                                rtecHandleServerError($outerWrap.length ? $outerWrap : $form.closest('.rtec'), {
                                    stopProcessing: $form,
                                    processingContext: 'area',
                                    enableButtons: $form.find('input[type=submit]')
                                });
                            }
                        }); // ajax
                    });
                ...
                $rtec.find('.rtec-unregister-link').on('click',function(event) {
                    event.preventDefault();
                    var $link = $(this);
                    if ($link.hasClass('rtec-processing')) {
                        return;
                    }
                    $link.addClass('rtec-processing');

                    var $wrapEl = $link.closest('.rtec-already-registered-options'),
                        $rtecWrap = $wrapEl.closest('.rtec-outer-wrap'),
                        submittedData = {
                            'event_id': $link.attr('data-event-id'),
                    ...
                    if (window.RtecCommon) {
                        window.RtecCommon.showLoading($wrapEl);
                    }
                    ...
                    $.ajax({
                        ...
                        success : function(data) {
                            ...
                            $link.removeClass('rtec-processing');
                            if (window.RtecCommon) {
                                window.RtecCommon.hideLoading($wrapEl);
                            }
                        },
                        error: function() {
            ...
            var RtecForm = {
                ...
                isDuplicateEmail: function (email, eventID, $context) {
                    var $emailEl = $context.find('input[name=rtec_email]'),
                        spinnerHtml = (window.RtecCommon && window.RtecCommon.spinnerHTML) ? window.RtecCommon.spinnerHTML() : '',
                        $spinner = '<span class="rtec-email-spinner">' + spinnerHtml + '</span>';
            ...
            $('.rtec-form').on('submit',function (event) {
                ...
                if (!$rtecEl.find('.rtec-error').length && !$rtecEl.find('#rtec-error-duplicate').length) {
                    var $formWrap = $rtecEl.find('.rtec-form-wrapper');
                    if (window.RtecCommon) {
                        window.RtecCommon.showLoading($formWrap);
                    }
                    $formWrap.find('#rtec-form, p').fadeTo(500, .3);
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            if (window.RtecCommon) {
                                window.RtecCommon.hideLoading($formWrap);
                            }
                            $rtecEl.find('#rtec-form-toggle-button').remove();
                        ...
                        error: function() {
                            ...
                            $formWrap.find('#rtec-form, p').fadeTo(500, 1);
                            $rtecEl.find('#rtec-form-toggle-button').css('visibility', '');
                            $rtecEl.find('input[name=rtec_submit]').prop('disabled', false).css('opacity', 1);
                            rtecHandleServerError($rtecEl);
                        }
                    }); // ajax
                ...
                if (window.RtecCommon) {
                    if (Array.isArray($elements)) {
                        $elements.forEach(function($element) {
                            window.RtecCommon.showLoading($element);
                        });
                    } else {
                        window.RtecCommon.showLoading($elements);
                    }
                }
            }
            ...
            function rtecSetToReady($elements) {
                if (window.RtecCommon) {
                    if (Array.isArray($elements)) {
                        $elements.forEach(function($element) {
                            window.RtecCommon.hideLoading($element);
                        });
                    } else {
                        window.RtecCommon.hideLoading($elements);
                    }
                }
            }
            ...
            $('#rtec-confirm-unregister-form').on('submit',function(event) {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if (window.RtecCommon) {
                            window.RtecCommon.stopProcessing($form, 'area');
                        }
                        ...
                        var $wrap = $form.closest('.rtec-outer-wrap').length ? $form.closest('.rtec-outer-wrap') : $('.rtec-outer-wrap').first();
                        if (eventId && $wrap.length) {
                            var evt = $.Event('rtecregistrationchange');
                            evt.el = $wrap;
                            evt.eventId = eventId;
                            $(window).trigger(evt);
                        }
                    ...
                    error: function() {
                        ...
                        $form.find('button').prop('disabled', false).css('opacity', 1);
                        rtecHandleServerError($form.closest('#rtec-confirm-unregister').length ? $form.closest('#rtec-confirm-unregister') : $('.rtec').first());
                    }
                }); // ajax

            });

            // On registration change: refresh attendee list and attendance count (submit, cancel, etc.)
            function rtecRefreshOnRegistrationChange(event) {
                var $rtecEl = event.el && event.el.closest('.rtec-outer-wrap').length ? event.el.closest('.rtec-outer-wrap') : (event.el || $());
                var eventId = (event.eventId != null && event.eventId !== '') ? String(event.eventId) : (typeof $rtecEl.find('.rtec').attr('data-event') !== 'undefined' ? $rtecEl.find('.rtec').attr('data-event') : (event.el && event.el.attr('data-event')));
                if (!eventId) { return; }
            ...
            $(window).on('rtecsubmissionajax', function (event) {
                ...
                if (eventId && $rtecEl.length) {
                    var changeEvt = $.Event('rtecregistrationchange');
                    changeEvt.el = $rtecEl;
                    changeEvt.eventId = eventId;
                    $(window).trigger(changeEvt);
                }
            });

            $(window).on('rtecregistrationchange', rtecRefreshOnRegistrationChange);
        }
    }
